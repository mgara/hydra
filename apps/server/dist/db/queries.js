...
export async function isSetupComplete() {
    try {
        const result = await getDb().execute('SELECT setup_complete FROM system_config WHERE id = 1');
...
export async function getGpioAssignments() {
...
export async function getGpioPinConfig() {
    const assignments = await getGpioAssignments();
    let masterValvePin = 0;
    let masterFlowPin = 0;
    let rainSensorPin = null;
    const zoneValvePins = {};
    const zoneFlowPins = {};
    for (const a of assignments) {
        switch (a.role) {
            case 'master_valve':
                masterValvePin = a.pin;
                break;
            case 'master_flow':
                masterFlowPin = a.pin;
                break;
            ...
            case 'zone_valve':
                if (a.zone != null)
                    zoneValvePins[a.zone] = a.pin;
                break;
            case 'zone_flow':
                if (a.zone != null)
                    zoneFlowPins[a.zone] = a.pin;
                break;
        }
    }
    return { masterValvePin, masterFlowPin, rainSensorPin, zoneValvePins, zoneFlowPins };
}
export async function executeSetup(input, assignments) {
    const db = getDb();
...
export async function updateZone(zone, name, enabled) {
    await getDb().execute({
        sql: `UPDATE zones SET name = ?, enabled = ?, updated_at = datetime('now') WHERE zone = ?`,
        args: [name, enabled ? 1 : 0, zone],
    });
}
export async function updateZoneProfile(zone, soilType, plantType, smartEnabled) {
...
// ── Schedule Queries ────────────────────────────────────
export async function getSchedules(zone) {
    if (zone) {
...
export async function getEnabledSchedules() {
    const result = await getDb().execute('SELECT * FROM schedules WHERE enabled = 1 ORDER BY start_time');
    return rowsToSchedules(result.rows);
}
...
export async function updateSchedule(id, input) {
    ...
    if (input.startMode !== undefined) {
        ...
        values.push(input.startMode);
    }
    if (input.startOffset !== undefined) {
        fields.push('start_offset = ?');
        values.push(input.startOffset);
    }
    ...
    if (input.rainSkip !== undefined) {
        fields.push('rain_skip = ?');
        values.push(input.rainSkip ? 1 : 0);
    }
...
export async function createLog(zone, trigger, scheduleId) {
    const result = await getDb().execute({
...
export async function completeLog(id, status, durationSeconds, volumeGallons, notes) {
    await getDb().execute({
...
export async function hasRecentLog(scheduleId, startTime) {
    const result = await getDb().execute({
...
export async function getRecentRainSkips() {
    const result = await getDb().execute(`
...
// ── Alert Queries ───────────────────────────────────────
export async function createAlert(severity, title, message) {
    const result = await getDb().execute({
...
export async function getAlerts(includeDismissed = false) {
    if (includeDismissed) {
        const result = await getDb().execute('SELECT * FROM alerts ORDER BY created_at DESC LIMIT 100');
        return result.rows;
    }
    const result = await getDb().execute('SELECT * FROM alerts WHERE dismissed = 0 ORDER BY created_at DESC LIMIT 100');
    return result.rows;
}
...
export async function insertFlowReading(gpm, pulseCount, zone, eventType = 'normal') {
    await getDb().execute({
...
export async function pruneOldFlowReadings(olderThanDays = 7) {
    await getDb().execute({
...
export async function insertSoilReading(device, moisture, temperature, battery, linkQuality, zone) {
    await getDb().execute({
...
export async function getLatestSoilReadingForZone(zone) {
    const result = await getDb().execute({
...
export async function getSoilDevices() {
    const result = await getDb().execute(`
...
export async function pruneOldSoilReadings(olderThanDays = 30) {
    await getDb().execute({
...
function rowToSchedule(row) {
    return {
        ...
        startMode: row.start_mode ?? 'fixed',
