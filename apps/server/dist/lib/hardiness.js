...
export function tempToZone(minTempF) {
    for (const z of HARDINESS_ZONES) {
        if (minTempF >= z.minTempF && minTempF < z.maxTempF)
            return z.code;
    }
    if (minTempF < -60)
        return '1a';
...
export function zoneToNumber(code) {
...
export function getRecommendedTurfType(zoneCode) {
...
export function getRecommendedPlantTypes(zoneCode) {
    const num = zoneToNumber(zoneCode);
    const recommended = [];
    // Turf recommendation
    if (num <= 6.5) {
    ...
    if (num >= 2 && num <= 8)
        recommended.push('evergreen_trees');
    if (num >= 3 && num <= 9.5)
        recommended.push('shade_trees');
    if (num >= 4 && num <= 9.5)
        recommended.push('fruit_trees');
    // Universal
    recommended.push('perennials', 'annuals', 'vegetable', 'native_plants');
    if (num >= 7)
        recommended.push('xeriscape');
    return recommended;
}
/** Check if a date is within the growing season for a zone */
export function isInGrowingSeason(zoneCode, date = new Date()) {
    const zone = getZone(zoneCode);
    if (!zone)
        return true; // assume growing season if unknown
 ...
 */
export async function detectHardinessZone(lat, lon) {
...
//# sourceMappingURL=hardiness.js.map