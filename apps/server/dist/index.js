...
function getWifiRssi() {
    try {
        if (process.platform === 'linux') {
            ...
            const raw = execSync('/System/Library/PrivateFrameworks/Apple80211.framework/Resources/airport -I', { timeout: 2000, encoding: 'utf8' });
...
async function startOperationalMode() {
    ...
    if (MQTT_BROKER) {
        try {
            ...
            mqttClient.on('soil:zone-wet', async (data) => {
                ...
                if (zoneManager.getRunningZones().includes(data.zone)) {
                    ...
                    await db.createAlert('info', 'Zone Stopped (Soil Wet)', `Zone ${data.zone} stopped — soil moisture ${data.moisture}% exceeds ${data.threshold}% threshold (sensor: ${data.friendlyName}).`);
                }
            ...
            mqttClient.on('soil:general-wet', async (data) => {
                if (zoneManager.isRainDelayed())
                ...
                if (!recentIrrigation) {
                    ...
                    await db.createAlert('info', 'Rain Detected (Soil Sensor)', `All irrigation paused 24h — sensor "${data.friendlyName}" reports ${data.moisture}% moisture with no irrigation in the past 24h.`);
                }
                else {
    ...
    if (OLED_ENABLED) {
        try {
            ...
            display.on('action', async (action) => {
                switch (action) {
                    case 'stop-all':
                        zoneManager.stopAll();
                        gpio.closeMaster();
