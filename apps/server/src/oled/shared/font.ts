...
// Each character is stored as 5 bytes (columns), MSB at top
const FONT_5X7: Record<number, number[]> = {
  ...
  103: [0x08, 0x54, 0x54, 0x54, 0x3C], // g
...
const CELL_WIDTH = CHAR_WIDTH + CHAR_SPACING; // 6px per char

export function drawChar(fb: FrameBuffer, x: number, y: number, ch: string, invert = false): void {
  const code = ch.charCodeAt(0);
  const glyph = FONT_5X7[code] ?? FONT_5X7[63]; // fallback to '?'
  if (!glyph) return;

  for (let col = 0; col < CHAR_WIDTH; col++) {
    const colData = glyph[col];
    for (let row = 0; row < CHAR_HEIGHT; row++) {
      const on = !!(colData & (1 << row));
      fb.setPixel(x + col, y + row, invert ? !on : on);
    }
  }
}

export function drawText(fb: FrameBuffer, x: number, y: number, text: string, invert = false): void {
  for (let i = 0; i < text.length; i++) {
    drawChar(fb, x + i * CELL_WIDTH, y, text[i], invert);
  }
}

export function textWidth(text: string): number {
  if (text.length === 0) return 0;
  return text.length * CELL_WIDTH - CHAR_SPACING;
}

export function drawTextRight(fb: FrameBuffer, x: number, y: number, text: string): void {
  drawText(fb, x - textWidth(text), y, text);
}

export function drawTextCenter(fb: FrameBuffer, y: number, text: string): void {
  const w = textWidth(text);
  drawText(fb, Math.floor((fb.width - w) / 2), y, text);
}
