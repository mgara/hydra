...
const LEGACY_GPIO_MAP: Record<number, number> = {
...
async function createFreshSchema(db: Client): Promise<void> {
...
async function detectLegacySchema(db: Client): Promise<boolean> {
  try {
    // Check if zones table exists with old CHECK constraint
    const tableInfo = await db.execute(
      "SELECT sql FROM sqlite_master WHERE type='table' AND name='zones'"
    );
    if (tableInfo.rows.length === 0) return false; // no zones table = fresh DB
...
async function migrateLegacySchema(db: Client): Promise<void> {
  ...
  try {
    ...
    const assignments: { role: string; pin: number; zone: number | null; label: string }[] = [
...
async function addSolarSchedulingColumns(db: Client): Promise<void> {
  try {
...
async function addSoilReadingsTable(db: Client): Promise<void> {
  try {
    await db.executeMultiple(`
      CREATE TABLE IF NOT EXISTS soil_readings (
        id            INTEGER PRIMARY KEY AUTOINCREMENT,
...
async function addZoneProfileColumns(db: Client): Promise<void> {
  try {
