...
import type { WeatherData, ForecastDay, HeatWaveStatus } from '../types.js';
...
let cachedSolarTimes: { sunrise: string; sunset: string } | null = null;
let solarCacheDate = '';
...
export async function shouldSkipForRain(): Promise<{ skip: boolean; probability: number }> {
 ...
 */
export async function getSolarTimes(): Promise<{ sunrise: string; sunset: string } | null> {
  const lat = parseFloat(await getSetting('weather_lat') ?? String(WEATHER_LAT));
  const lon = parseFloat(await getSetting('weather_lon') ?? String(WEATHER_LON));
  const latLon = `${lat},${lon}`;
...
export function computeSolarTime(
  baseTime: string, // "HH:MM" from sunrise or sunset
 ...
 */
function detectHeatWave(forecast: ForecastDay[], thresholdF: number): HeatWaveStatus {
  ...
  for (const day of forecast) {
    if (day.temperatureHighF >= thresholdF) {
      consecutiveDays++;
      if (day.temperatureHighF > peakTempF) peakTempF = day.temperatureHighF;
    } else {
