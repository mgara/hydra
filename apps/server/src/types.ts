...
export interface Schedule {
  ...
  zone: number;
  name: string;
  startTime: string;      // HH:MM (for fixed mode, or last computed time for solar modes)
  startMode: StartMode;   // 'fixed' = exact time, 'sunrise'/'sunset' = relative to sun
  startOffset: number;    // minutes offset for sunrise/sunset (negative = before, positive = after)
  durationMinutes: number;
  days: string;            // comma-separated: "mon,tue,wed,thu,fri"
  enabled: boolean;
  rainSkip: boolean;
...
export interface HeatWaveStatus {
  ...
  consecutiveDays: number;    // days above threshold in forecast
  peakTempF: number | null;   // highest forecasted temp
  boostMultiplier: number;    // 1.0 = no boost, 1.3 = 30% more water
}
...
export interface ForecastDay {
  ...
  weatherCode: number;
  shouldSkip: boolean;
  et0Mm: number | null;  // FAO-56 reference evapotranspiration (mm/day)
}
