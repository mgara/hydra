...
import type { SetupInput, GpioBudget, GpioAssignment } from '../types.js';
 ...
 */
export function calculateGpioBudget(input: SetupInput): GpioBudget {
  const pool = [...AVAILABLE_GPIO_POOL];
  const assignments: Omit<GpioAssignment, 'id'>[] = [];
  let idx = 0;
