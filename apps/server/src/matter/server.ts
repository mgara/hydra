...
function setValveState(ep: Endpoint, patch: Partial<{
  currentState: ValveConfigurationAndControl.ValveState | null;
  targetState: ValveConfigurationAndControl.ValveState | null;
...
export async function startMatterServer(zoneManager: ZoneManager): Promise<MatterBridge> {
  ...
  const valveEndpoints = new Map<number, Endpoint>();
  for (const state of zoneStates) {
    const endpoint = createValveEndpoint(
      zoneManager,
      state.zone,
      state.name,
  ...
  const onZoneStart = (zoneState: ZoneState) => {
  ...
  const onZoneStop = (zoneState: ZoneState) => {
    const ep = valveEndpoints.get(zoneState.zone);
    if (!ep) return;
    setValveState(ep, {
