...
export class WifiManager extends EventEmitter {
  private status: WifiStatusCode = 0;
  ...
  async connect(ssid: string, password: string): Promise<WifiStatusCode> {
  ...
  private connectLinux(ssid: string, password: string): Promise<void> {
