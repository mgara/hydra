...
export function registerSystemRoutes(
  ...
  app.get<{ Querystring: { all?: string } }>('/api/system/alerts', async (req) => {
    return await db.getAlerts(req.query.all === 'true');
  });

  // POST /api/system/alerts/:id/dismiss — dismiss alert
  app.post<{ Params: { id: string } }>('/api/system/alerts/:id/dismiss', async (req) => {
  ...
  app.post<{ Body: { state: 'open' | 'closed' } }>('/api/system/master-valve', async (req, reply) => {
  ...
  app.post<{ Body: { hours?: number; clear?: boolean } }>('/api/system/rain-delay', async (req) => {
    if (req.body.clear) {
  ...
  app.post('/api/system/force-shutdown', async () => {
    zoneManager.stopAll();
    gpio.closeMaster();
    await db.createAlert('critical', 'Force Shutdown',
      'All zones stopped and master valve closed via force shutdown.');
    return { success: true, message: 'All zones stopped, master valve closed' };
  });
}

function getCpuTemp(): number | null {
  if (!IS_PRODUCTION) return 42 + Math.random() * 5; // mock
  try {
