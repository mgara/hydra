...
export function registerZoneRoutes(app: FastifyInstance, zoneManager: ZoneManager): void {
  ...
  app.get<{ Params: { zone: string } }>('/api/zones/:zone', async (req, reply) => {
  ...
  app.put<{ Params: { zone: string }; Body: { name?: string; enabled?: boolean } }>(
    '/api/zones/:zone',
    async (req, reply) => {
      const zone = parseInt(req.params.zone, 10);
      if (!zoneManager.isValidZone(zone)) {
      ...
      await db.updateZone(
        ...
        req.body.enabled ?? !!existing.enabled,
      );
  ...
  app.put<{ Params: { zone: string }; Body: { soilType?: string | null; plantType?: string | null; smartEnabled?: boolean } }>(
    '/api/zones/:zone/profile',
    async (req, reply) => {
      const zone = parseInt(req.params.zone, 10);
      if (!zoneManager.isValidZone(zone)) {
        return reply.status(400).send({ error: `Invalid zone (1-${zoneManager.getZoneCount()})` });
      ...
      let smartSchedule: { created: boolean; reason?: string } | undefined;
  ...
  app.get<{ Params: { zone: string } }>('/api/zones/:zone/smart', async (req, reply) => {
    const zone = parseInt(req.params.zone, 10);
    if (!zoneManager.isValidZone(zone)) {
      return reply.status(400).send({ error: `Invalid zone (1-${zoneManager.getZoneCount()})` });
    }
  ...
  app.put<{ Body: { zone: string } }>('/api/zones/hardiness', async (req) => {
    const zoneData = getZone(req.body.zone);
    if (!zoneData) return { error: 'Invalid zone code' };
  ...
  app.post<{ Params: { zone: string }; Body: { durationMinutes?: number } }>(
    ...
    async (req, reply) => {
      ...
      if (!result.success) return reply.status(409).send({ error: result.error });
      return zoneManager.getZoneState(zone);
    }
  );

  // POST /api/zones/:zone/stop — stop a zone
  app.post<{ Params: { zone: string } }>('/api/zones/:zone/stop', async (req, reply) => {
    const zone = parseInt(req.params.zone, 10);
    if (!zoneManager.isValidZone(zone)) {
      return reply.status(400).send({ error: `Invalid zone (1-${zoneManager.getZoneCount()})` });
    }
